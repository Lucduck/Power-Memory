...
export default class extends Phaser.Sprite {
  constructor ({game, x, y, asset, scored, plusTime, soundActivated, typeGame}) {
  ...
  placeTiles () {
    ...
    if (this.typeGame === 3) { // X3
      topSpace += 80
    }

    for (let i = 0; i < this.numRows * this.numCols; i++) {
      this.tilesArray.push(Math.floor(i / this.typeGame)) // X3
    }
  ...
  showTile (target) {
    if (this.selectedArray.length < this.typeGame && this.selectedArray.indexOf(target) === -1) { // X3
      this.playSound(this.sounds.select)
      target.frame = target.value
      this.selectedArray.push(target)
      if (this.selectedArray.length === this.typeGame && this.checked === true) { // X3
        this.checked = false
  ...
  checkTiles () {
    ...
    if (conditionCheck) {
      if (this.selectedArray[0].value === this.selectedArray[1].value) {
        ...
        this.scored()
        this.plusTime()
        for (var i = 0; i < this.typeGame; i++) { // X3
          this.selectedArray[i].destroy()
        }
        this.tilesLeft -= this.typeGame // X3
        if (this.tilesLeft === 0) {
